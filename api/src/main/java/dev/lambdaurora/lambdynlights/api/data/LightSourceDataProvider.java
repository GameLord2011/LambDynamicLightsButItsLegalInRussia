...
public abstract class LightSourceDataProvider<L, C extends LightSourceDataProvider<L, C>.Context>
			...
			return CompletableFuture.allOf(
					context.sources.entrySet().stream()
							.map(entry -> {
								...
								var result = codec.encodeStart(ops, entry.getValue()).getOrThrow(false, error -> {});

								return DataProvider.saveStable(cachedOutput, result, this.pathProvider.json(entry.getKey()));
	...
	protected abstract class Context {
		...
		public @NotNull Identifier idOf(@NotNull String path) {
			return new Identifier(LightSourceDataProvider.this.defaultNamespace, path);
		}
